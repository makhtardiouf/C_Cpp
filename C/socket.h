...
#define sync_sendto(s, buffer, bufsize, addr)                                  \
  async_sendto(s, buffer, bufsize, addr, -1)

#define sync_recvfrom(s, buffer, bufsize, addr)                                \
  async_recvfrom(s, buffer, bufsize, addr, -1)
...
extern int get_socket_name(struct sockaddr_in *name, char *hostname,
                           char *hostaddr);

extern SOCKET create_socket(char *hostname, unsigned short port, char *protocol,
                            int action);
...
extern int async_send(SOCKET s, char *buffer, int bufsize, int timeout);

extern int async_recv(SOCKET s, char *buffer, int bufsize, int timeout);
...
extern int async_connect(SOCKET s, struct sockaddr_in *addr, int timeout);

extern SOCKET async_accept(SOCKET s, struct sockaddr_in *addr, int timeout);
#ifdef __cplusplus
}
#endif
#endif
