...
struct element {
  label_t label : LABEL_BITS;
  chunk_t chunk : CHUNK_BITS;
} PACKED;
struct version {
  tag_t tag;
  label_t parent;
  bool used, ghost, present, pathmap, nearmap;
};
...
int show_subtree(struct sb *sb, version_t version, int depth,
                 version_t target) {
  struct version *table = sb->table;
  ...
  if (!is_ghost(sb, version))
    ...
    printf("~%i", sb->child_count[version]);
  for (int i = 0; i < element_count; i++)
...
int count_tree(struct sb *sb) { return count_subtree(sb, 0, 0); }
...
label_t new_version(struct sb *sb, label_t parent, uint32_t tag) {
  ...
  sb->child_index[sb->version_count] =
      sb->version_count ? sb->child_index[last] + sb->child_count[last]
                        : sb->children;
...
label_t *find_child_pos(struct sb *sb, version_t parent, version_t child,
                        unsigned count) {
...
bool delete_elements(struct sb *sb, version_t target, version_t parent) {
  ...
  struct element *limit = elements + element_count, *save = elements,
                 *kill = NULL;
  ...
  if (!is_present(sb, target)) {
    ...
    if (ancestor && is_ghost(sb, ancestor) && is_present(sb, ancestor) &&
        !count_heirs(sb, ancestor))
...
void show_tree_with_target(struct sb *sb, tag_t tag) {
  ...
  int total =
      sb->child_count[0] ? show_subtree(sb, get_root(sb), 0, target) : 0;
  printf("%u versions\n", total);
}
...
void same_test(struct sb *sb, version_t version1, version_t version2) {
  printf("%u and %u are %s\n", version1, version2,
         same_chunk(sb, version1, version2) ? "the same" : "different");
}
