...
int get_socket_name(struct sockaddr_in *name, char *hostname, char *hostaddr) {
  struct hostent *hp;
  hp = NULL;
  int i;

  hp =
      gethostbyaddr((char *)&(name->sin_addr), sizeof(name->sin_addr), AF_INET);
  ...
  for (i = 0; hp->h_addr_list[i]; i++) {
    sprintf(hostaddr, "%d.%d.%d.%d", (unsigned char)hp->h_addr_list[i][0],
...
SOCKET create_socket(char *hostname, unsigned short port, char *protocol,
                     int action) {
  SOCKET s;
  struct hostent *hp;
...
SOCKET broad_cast_socket(void) {
  ...
  if (setsockopt(s, SOL_SOCKET, SO_BROADCAST, (SOCKOPT_TYPE *)&optval,
                 sizeof(int)))
    return INVALID_SOCKET;
...
int async_recv_from(SOCKET s, char *msg, int msg_size, struct sockaddr_in *addr,
                    int timeout) {
  clock_t endtime;
  int rec;
  int addr_size;
...
int async_send_to(SOCKET s, char *msg, int msg_size, struct sockaddr_in *addr,
                  int timeout) {
  clock_t endtime;
  int snd;
