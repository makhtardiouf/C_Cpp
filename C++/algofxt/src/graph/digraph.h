...
public:
    ...
    // By default implicitly represent the nodes as values p with 0 ≤ p < N
    
    ulong *ep_;  // e[ep[k]], ..., e[ep[k+1]-1]: outgoing connections of node k
    ulong *e_;   // outgoing connections (Edges)
    
    // otherwise use this to store values, then node k must correspond to vn[k]
    // sort it after Initialization for bin searching
...
public:
    ...
    ulong C(ulong p, ulong pn)  const;
