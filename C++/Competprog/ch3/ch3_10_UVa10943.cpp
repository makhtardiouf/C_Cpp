...
int ways(int N, int K) {
  ...
  // if K > 1, we can choose one number from [0..N] to be one of the number and
recursively compute the rest
  int total_ways = 0;
  for (int split = 0; split <= N; split++)
    total_ways = (total_ways + ways(N - split, K - 1)) % 1000000; // we just
need the modulo 1M
...
int main() {
  ...
  while (scanf("%d %d", &N, &K), (N || K)) // some recursion formula + top down
DP
    printf("%d\n", ways(N, K));
  return 0;
}
...
int main() {
  ...
  for (j = 1; j < 100;
       j++) // these three nested loops form the correct topological order
    for (i = 0; i <= 100; i++)
