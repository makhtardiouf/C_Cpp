...
int main() {
  ...
  prev_dash = this_dash = false; // to differentiate whether the previous line
  ...
  while (1) {
    ...
    if (line[(int)strlen(line) - 1] == '-') {
      line[(int)strlen(line) - 1] = 0; // if the last character is '-', delete
                                       // it by moving the NULL (0) one
                                       // character forward
      this_dash = true;
    } else
      this_dash = false;
  ...
  for (i = digits = alphas = vowels = consonants = 0; str[i];
  ...
  printf(
      "%s %s\n", tokens[0].c_str(),
      tokens[(int)tokens.size() - 1]
          .c_str()); // to cast C++ string to C string, we need to use c_str()
  printf("%d\n", hascs3233);
